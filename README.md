...
| **`AtmSimulator/`** <br>*(Atm, OptionMenu)* | OOP Inheritance, Encapsulation, Exception Handling | **ATM Terminal Simulator:** A CLI banking application simulating an ATM interface. Features secure login validation, dual-account management (Checking & Savings), transaction routing, and robust `try-catch` blocks to prevent input mismatch crashes. |
